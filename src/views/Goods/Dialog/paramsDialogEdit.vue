<template>
  <div>
    <el-dialog
      :title="flagStatus=='many'?'修改动态参数':'修改静态属性'"
      width="650px"
      @opened="diaOpen"
      :visible="isVisible"
      @close="close"
...
export default {
  ...
  methods: {
      ...
      GetcategoryParamsById(
        {
          ...
          attrId: this.currentAttrId
        },
        {
          attr_sel: this.flagStatus
        }
      ).then(res => {
        this.form.attr_name = res.data.data.attr_name;
      });
    },
    //打开dialog框
    faControlOpen() {
    ...
    //父组件改变我的状态
    faControlStatus(status, categoryId, attrId) {
      this.flagStatus = status;
      this.currentCategoryToparamsId = categoryId;
